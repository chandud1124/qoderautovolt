...
class DisplayManager:
    def __init__(self, storage_manager: LocalStorageManager):
        ...
        self.current_notices = []  # Only active content from server
        self.current_notice_index = 0
        self.last_notice_change = datetime.now()
        self.running = True
        self.image_cache = {}  # Cache for downloaded images
        self.current_content_id = None  # Track currently displayed content
        self.api_manager = None  # Will be set later
    ...
    def display_notice(self, notice: Dict[str, Any]):
        ...
        if notice.get('_id'):
            ...
            # Report current content to backend if it changed
            if self.current_content_id != notice['_id']:
                self.current_content_id = notice['_id']
                if self.api_manager:
                    content_info = {
                        'id': notice.get('_id'),
                        ...
                        'duration': notice.get('schedule', {}).get('duration', 60)
                    }
                    self.api_manager.update_board_current_content(content_info)
        ...
        if 'attachments' in notice and notice['attachments']:
            for attachment in notice['attachments']:
                mimetype = attachment.get('mimetype') or attachment.get('mimeType') or ''
                if not mimetype:
                    mimetype = self.guess_mimetype(attachment.get('originalName', ''))
                
                logging.info(f"Checking attachment: {attachment.get('originalName')} - {mimetype}")
                if mimetype.startswith('image/'):
                    ...
                    if image_url.startswith('/'):
                        ...
                        # If it's a full URL, replace localhost with the actual server host
                        if 'localhost' in image_url:
                            server_host = Config.SERVER_URL.split('://')[1].split(':')[0]
                            image_url = image_url.replace('localhost', server_host)
                    ...
                    # Handle PDF by rendering first page as image
                    ...
                    if pdf_url.startswith('/'):
                        ...
                        if 'localhost' in pdf_url:
                            server_host = Config.SERVER_URL.split('://')[1].split(':')[0]
                            pdf_url = pdf_url.replace('localhost', server_host)
                    
                    logging.info(f"Downloading PDF from: {pdf_url}")
                    pdf_image = self.download_pdf_first_page(pdf_url)
                    if pdf_image:
                        # Display PDF first page as image
                        image_y = 150
                        image_height = Config.DISPLAY_HEIGHT - 250
                        self.display_image(pdf_image, 50, image_y, Config.DISPLAY_WIDTH - 100, image_height)
                        has_image = True
                        logging.info(f"Displayed PDF: {attachment['originalName']}")
                        break
                elif mimetype.startswith('video/'):
                    # For videos, display a message since we can't play them in pygame easily
                    self.draw_text(f"Video Content: {attachment.get('originalName', 'Unknown')}", self.font_content, Config.TEXT_COLOR, 50, 200)
                    logging.info(f"Video attachment detected: {attachment['originalName']} - displaying text message")
                    break
                else:
                    # For other file types, display filename
                    self.draw_text(f"File: {attachment.get('originalName', 'Unknown')} ({mimetype})", self.font_content, Config.TEXT_COLOR, 50, 200)
                    logging.info(f"Unsupported attachment type: {attachment['originalName']} - {mimetype}")
                    break
        else:
    ...
    def download_image(self, url: str) -> Optional[pygame.Surface]:
        ...
        try:
            ...
            if not content_type.startswith('image/'):
                logging.error(f"URL does not return an image, content-type: {content_type}")
                if len(response.content) > 0:
                    logging.info(f"First 200 bytes: {response.content[:200]}")
                return None
    ...
    def download_pdf_first_page(self, url: str) -> Optional[pygame.Surface]:
        """Download PDF and render first page as image"""
        try:
            ...
            # Convert to PIL Image then to pygame
            img_data = pix.tobytes("png")
            img = pygame.image.load(io.BytesIO(img_data))
            
            logging.info(f"PDF first page rendered successfully, size: {img.get_size()}")
            return img
            
        except Exception as e:
            logging.error(f"Failed to render PDF from {url}: {e}")
            return None
    ...
    def update_content(self, server_content_data: Optional[Dict[str, Any]] = None):
        """Update content from server - only play what's currently active"""
        if server_content_data:
            # Extract active content from server response
            scheduled_content = server_content_data.get('scheduledContent', [])
            board_notices = server_content_data.get('boardNotices', [])
            group_content = server_content_data.get('groupContent', [])
            
            # Combine all active content
            active_content = scheduled_content + board_notices + group_content
            
            if active_content:
                # Sort by priority (highest first) and implement weighted rotation
                active_content.sort(key=lambda x: x.get('priority', 0), reverse=True)
                
                # Create a weighted rotation list based on priority
                # Higher priority content appears more frequently in the rotation
                weighted_notices = []
                for notice in active_content:
                    priority = max(1, notice.get('priority', 1))  # Minimum weight of 1
                    # Higher priority gets more slots in rotation (capped to prevent excessive repetition)
                    weight = min(5, priority)  # Cap at 5 to balance rotation
                    weighted_notices.extend([notice] * weight)
                
                self.current_notices = weighted_notices
                # Don't reset index if in manual control mode
                if not self.manual_control:
                    self.current_notice_index = 0
                    self.last_notice_change = datetime.now()
                
                logging.info(f"Loaded {len(self.current_notices)} active notices from server (weighted rotation with {len(set(n.get('_id', n.get('id', '')) for n in self.current_notices))} unique items)")
            else:
                self.current_notices = []
                self.display_no_content()
        else:
            # Fallback to local storage if no server data (offline mode)
            logging.warning("No server content data available, using local storage as fallback")
            ...
            if self.current_notices:
                ...
                if not self.manual_control:
                    self.current_notice_index = 0
                    self.last_notice_change = datetime.now()
                logging.info(f"Using local content as fallback: {len(self.current_notices)} items")
            else:
                self.display_no_content()

    def next_content(self):
        """Manually move to next content item"""
        if self.current_notices:
            self.current_notice_index = (self.current_notice_index + 1) % len(self.current_notices)
            self.manual_control = True
            self.last_notice_change = datetime.now()
            logging.info(f"Manual control: moved to next content (index {self.current_notice_index})")
            return True
        return False

    def previous_content(self):
        """Manually move to previous content item"""
        if self.current_notices:
            self.current_notice_index = (self.current_notice_index - 1) % len(self.current_notices)
            self.manual_control = True
            self.last_notice_change = datetime.now()
            logging.info(f"Manual control: moved to previous content (index {self.current_notice_index})")
            return True
        return False

    def set_content_index(self, index: int):
        """Set specific content index for manual control"""
        if self.current_notices and 0 <= index < len(self.current_notices):
            self.current_notice_index = index
            self.manual_control = True
            self.last_notice_change = datetime.now()
            logging.info(f"Manual control: set content index to {index}")
            return True
        return False

    def enable_auto_rotation(self):
        """Re-enable automatic content rotation"""
        self.manual_control = False
        self.last_notice_change = datetime.now()
        logging.info("Auto rotation re-enabled")
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.current_notices:
                # Only auto-cycle if not in manual control mode
                if not self.manual_control:
                    now = datetime.now()
...
class APIManager:
    ...
    def update_board_current_content(self, content_info: Dict[str, Any]):
        """Report currently displayed content to the backend for live preview sync"""
        try:
            url = f"{Config.SERVER_URL}/api/boards/{Config.BOARD_ID}/current-content"
            data = {
                'contentId': content_info.get('id'),
                'contentType': 'scheduled',
                'title': content_info.get('title'),
                'priority': content_info.get('priority'),
                'startedAt': datetime.now().isoformat(),
                'duration': content_info.get('duration', 60)
            }
            response = self.session.patch(url, json=data, headers=self.get_headers(), timeout=10)

            if response.status_code == 200:
                logging.info(f"Current content reported to backend: {content_info.get('title')}")
                return True
            else:
                logging.warning(f"Failed to report current content: HTTP {response.status_code}")
                return False

        except requests.RequestException as e:
            logging.error(f"Current content report failed: {e}")
            return False

    def control_content_display(self, action: str) -> Optional[Dict[str, Any]]:
        """Send content control command to backend (next/previous/set-index)"""
        try:
            url = f"{Config.SERVER_URL}/api/boards/{Config.BOARD_ID}/control-content"
            data = {
                'action': action,
                'timestamp': datetime.now().isoformat()
            }
            response = self.session.post(url, json=data, headers=self.get_headers(), timeout=10)

            if response.status_code == 200:
                result = response.json()
                if result.get('success'):
                    logging.info(f"Content control action '{action}' executed successfully")
                    return result.get('data')
                else:
                    logging.warning(f"Content control failed: {result.get('message')}")
            else:
                logging.warning(f"Failed to control content: HTTP {response.status_code}")

        except requests.RequestException as e:
            logging.error(f"Content control request failed: {e}")
...
class MQTTManager:
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            self.connected = True
            # Subscribe to notice published topic and content control topic
            self.client.subscribe(Config.MQTT_TOPIC)
            self.client.subscribe(f"boards/{Config.BOARD_ID}/control")
            logging.info(f"Subscribed to topics: {Config.MQTT_TOPIC}, boards/{Config.BOARD_ID}/control")
        else:
    ...
    def on_message(self, client, userdata, msg):
        try:
            ...
            if msg.topic == Config.MQTT_TOPIC:
                ...
                api_manager = APIManager(self.display_manager.storage_manager)
            ...
            elif msg.topic == f"boards/{Config.BOARD_ID}/control":
                # Handle content control commands
                action = payload.get('action')
                if action == 'next':
                    self.display_manager.next_content()
                elif action == 'previous':
                    self.display_manager.previous_content()
                elif action == 'set_index':
                    index = payload.get('index', 0)
                    self.display_manager.set_content_index(index)
                elif action == 'enable_auto':
                    self.display_manager.enable_auto_rotation()
                elif action == 'refresh':
                    ...
                    if content:
                        self.display_manager.update_content(content)
                        logging.info("Content refreshed via control command")

                logging.info(f"Content control action executed: {action}")
...
def main():
    ...
    display_manager.api_manager = api_manager  # Set API manager for content reporting
    ...
    try:
        while display_manager.running:
            ...
            if (current_time - api_manager.last_content_update).seconds >= Config.CONTENT_UPDATE_INTERVAL:
                content = api_manager.get_board_content()
                if content:
                    # Update display with fresh active content from server
                    display_manager.update_content(content)
                    api_manager.last_content_update = current_time
                    logging.info("Active content synced from server")
                else:
                    # Server unavailable, keep current content
                    logging.info("Server unavailable, keeping current content")
